...
**Last verified:** 2026-07-29 against `main` commit `23f5820be15696a495415493f5ad5be6be6bdcb2` (the latest commit before this delivery) and GitHub Actions state.
...
`0048_create_catalog_publication_governance.sql` is delivered in this session under the
...
|---|---|---|
| Migrations `0001`–`0048` | **Merged** | 48 migrations, 110 tables, 33,909 lines; see `docs/MIGRATION_INDEX.md`. |
...
| Database testing | **Merged but incomplete historically** | pgTAP covers `0042`–`0048`; migrations `0001`–`0041` lack dedicated behavioral suites. Database CI replays the full migration history. |
| Security / RLS | **Merged through `0048`** | RLS covers all 110 merged tables. The first-admin bootstrap procedure still requires manual exercise in a real local/staging project. |
...
| Catalog publication governance | **Merged (`0048`)** | Typed catalog versions, publication requests linked to the existing generic approval engine, two-person ordered review/final approval, scheduling/effective windows, publication/suspension/archive/rejection transitions, overlap prevention, append-only domain events, central audit integration, and rollback/supersession lineage. Uses the existing platform-wide `CATALOG_MANAGE` gate pending resource-scoped authorization in `0049`. |
...
None beyond this delivery. Migration `0048` is complete; do not start
migration `0049` until it is separately authorized. The roadmap's next
candidate is `0049` (Catalog Admin Authorization) — see
