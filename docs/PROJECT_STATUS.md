...
|---|---|---|
| Migrations `0001`–`0051` | **Database Phase complete** | 51 migrations, 111 tables, 35,219 lines; see `docs/MIGRATION_INDEX.md`. |
...
| Database testing | **Current migrations covered; historical gap remains** | 20 pgTAP files contain 506 assertions covering `0042`–`0051`; migrations `0001`–`0041` lack dedicated behavioral suites. Database CI replays the full migration history. |
| Security / RLS | **Complete through `0051`** | RLS covers all 111 tables. The detail and list/search boundaries are execute-only, snapshot-based, and read-only; they add no table grant or write path. Catalog scope behavior from `0049` remains unchanged. |
...
| Frontend | **Phase 3 in progress** | P3.1 homepage, P3.2 browsing, and P3.3 card details are green. P3.4 search plus bank/network/fee/persona/salary filters are delivered; application integration with the new reward/list interface is next. |
...
`0051`; P3.4 application integration is the next active delivery.
